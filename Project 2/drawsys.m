function drawsys(s,params)
x = s(1);
t1 = s(3);
t2 = s(5);

% dimensions
W = 1*sqrt(params.M/1000);  % cart width
H = .5*sqrt(params.M/1000); % cart height
wr = .2; % wheel radius
mr1 = .3*sqrt(params.m1/100);% mass 1 radius
mr2 = .3*sqrt(params.m2/100);% mass 2 radius
...
px1 = x + params.l1*sin(t1)/10;
py1 = y - params.l1*cos(t1)/10;
px2 = x + params.l2*sin(t2)/10;
py2 = y - params.l2*cos(t2)/10;
